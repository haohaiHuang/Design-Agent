...
- **交付物**：设计产物（HTML 原型/demo/幻灯片/信息图/动画）+ 关键决策记录
  （候选 → 选择理由 → 约束集 → 校验结果）。归档到项目 designs/ 等子目录。
- **默认不进入开发**：设计完成 ≠ 开始开发。除非用户**显式**要求（"开始开发/实现上线/继续做下去"），
  否则本流程止于设计交付——不写生产代码、不搭后端、不做工程化。
- **转开发的条件与方式**：用户要求开发时，产出**开发交接包**（约束集/DESIGN.md、组件与交互规格、
  设计文件清单），建议交给开发 Agent/流程执行；不默认自己动手写生产代码。
- **收尾动作**：确认交付物齐全归档 → 极简总结（做了什么、caveats、下一步可选方向，含"如需开发可转交"）。
