...
2. **`designs/` 已有哪些产物**：`brief.md` / `assets.md` / `route.md` / `tokens-*.md` / `candidates.md` / `constraints.md` / 产物 HTML / 建议文档
3. **三步定入口**：
   - **① 选链**：按需求类型 + 用户给了什么，定 L0/L1/L2 与对应的 op 链（见「三档粒度」）
   - **② 链内定位**：取链内**第一个「输出缺失」的 op** 作为入口
     （例：链内 `constraints.md` 在、产物 HTML 不在 → 入口是 `3.build`，不重跑前面的 op）
   - **③ 条件 op 只在触发时进链**：`0.facts` 仅当需求含产品/品牌/版本/规格时进链——它没有输入产物，
     若不加此限制会变成**所有任务的入口**（干跑模拟实测到的坑，见 §7 GAP-8）；L0 请求不进 L2 链，
